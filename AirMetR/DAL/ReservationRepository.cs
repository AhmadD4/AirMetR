...
namespace AirMetR.DAL
{
    public class ReservationRepository : IReservationRepository
    {
        ...
        public async Task<IEnumerable<Reservation>?> GetReservationsByUserId(string userId)
        {
            try
            {
                var reservations = await _db.Reservations.Where(r => r.CustomerId == userId).ToListAsync();
                return reservations;
            }
            catch (Exception e)
            {
        ...
        public async Task<Reservation?> GetReservationByUserIdAndPropertyId(string userId, int propertyId)
        {
            try
            {
                return await _db.Reservations.Where(r => r.CustomerId == userId && r.PropertyId == propertyId).FirstOrDefaultAsync();
            }
            catch (Exception e)
            {
